...
class MeshviewerGenerator
{
    ...
    private function buildNodesForMeshviewerList()
    {
        ...
        foreach ($devices as $device) {
            ...
            $node['gateway_nexthop'] = $_ENV['GATEWAY_NEXTHOP'];
    ...
    {
        try {
            return json_decode(@file_get_contents($_ENV['FREIFUNK_MESHVIEWERURL']), true);
        } catch (\Exception $ex) {
    ...
    private function getGatewayMac()
    {
        if (is_null($this->gateway)) {
            $this->loadGateway($_ENV['GATEWAY_NEXTHOP']);
        }
    ...
    public function writeDeviceFile($device)
    {
        if ($this->checkDeviceFileExists($device->serial)) {
            if ($device->state == 1) {
                ...
                $deviceData['owner'] = $_ENV['OWNER_EMAIL'];
            }
        } else {
            $deviceData = [];
            $deviceData['name'] = '';
